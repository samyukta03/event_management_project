...
public class Home_page extends JFrame {
    ...
    ArrayList<String> listofevents = new ArrayList<String>() {
        {
            ...
            add("Code n Chaos");add("Play with data");add("Paper Presentation");
            add("Workshop on Blockchain+web3");add("Web on");add("OSPC");
            add("PyDon");add("IEEE WorkShop");add("FastestFinger");
   ...
   /* ArrayList<String> listofevents = new ArrayList<String>();
    public void listofevents(){
        ...
        ResultSet rs = null;
        try {
            con1 = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","system",
                    "samyu");
//            if(con1!=null){
//                System.out.println("DB Conn");} else System.out.println("fail");
            String sql = "select event from events";
            st = con1.prepareStatement(sql);
            rs = st.executeQuery();
            int i=0;
            while(rs.next()){
                String a = rs.getString("event");
                listofevents.add(a);
            }
...
//                System.out.println(item);
                ...
                if (item == "Code n Chaos") {
                    ...
                    Timing = "4 Hours";
                    venue = "CT Dept";
                    prizes=" Internships for winners ";
                    extra = " Bring your own systems ";
                }
...
//                    System.out.println(item);
            ...
            //particular event is also created with columns like name
                ...
                try {
                    ...
                    int row = stmnt.executeUpdate();
                    System.out.println(String.format("Details added %d", row));
                    String status = event + "Registered \n";
                    status += ">>> Event Details:" + "\n";
                    status += ">Date : " + doe + "\n";
                    status += ">Time : " + time + "\n";
                    status += ">Timing : " + timing + "\n";
